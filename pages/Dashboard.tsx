  ...
  return (
    <div className="space-y-10 max-w-7xl mx-auto">
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
        <StatCard title={t("待审核总量")} value="128" trend="12%" isPositive={false} />
        <StatCard title={t("AI 辅助已完成")} value="4,291" trend="18.4%" isPositive={true} />
        <StatCard title={t("今日处理数")} value="32" trend="25%" isPositive={true} />
        <StatCard title={t("平均准确率")} value="98.5%" trend="0.8%" isPositive={true} />
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-10">
        <div className="lg:col-span-2 apple-card p-8">
          <div className="flex items-center justify-between mb-10">
            <div>
              <h4 className="text-xl font-bold text-[#1D1D1F]">{t("系统负载分析")}</h4>
              <p className="text-xs text-gray-400 mt-1">{t("过去 7 天的审核请求流量波动图")}</p>
            </div>
            <select className="bg-gray-100 border-none rounded-xl text-xs font-bold px-4 py-2 text-gray-600 focus:ring-0 cursor-pointer">
              <option>{t("本周数据")}</option>
              <option>{t("本月数据")}</option>
            </select>
          </div>
        ...
        <div className="apple-card p-8 flex flex-col">
          <h4 className="text-xl font-bold text-[#1D1D1F] mb-8">{t("最新动态")}</h4>
          <div className="space-y-8 flex-1">
            {[
              { title: t('《星际穿越》重映版 AI 复核通过'), time: t('12 分钟前'), risk: t('低风险'), color: 'bg-green-500' },
              { title: t('检测到 《未名记录》 存在版权冲突'), time: t('28 分钟前'), risk: t('中风险'), color: 'bg-amber-500' },
              { title: t('《大红灯笼高高挂》数字化修复审核中'), time: t('1 小时前'), risk: t('处理中'), color: 'bg-blue-500' },
              { title: t('用户 092 提交了新的审核申请'), time: t('2 小时前'), risk: t('待处理'), color: 'bg-gray-300' },
            ].map((item, i) => (
