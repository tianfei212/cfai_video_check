...
const AuditDetail: React.FC = () => {
  ...
  // 从 compareVersions 中获取第一个待比对的版本名称（如果有的话）
  const targetName = useMemo(() => {
    if (compareVersions && Object.keys(compareVersions).length > 0) {
      return Object.values(compareVersions)[0];
    }
    return undefined;
  }, [compareVersions]);
        ...
        <section className="flex-1 grid grid-cols-3 gap-6 min-h-0">
          ...
          <div className="flex flex-col gap-3">
             <div className="flex-1 rounded-[2.5rem] bg-black border border-white/5 overflow-hidden relative shadow-2xl group">
                ...
                <div className="absolute top-6 left-6 px-3 py-1 bg-black/60 backdrop-blur-md rounded-xl text-[10px] font-bold text-white border border-white/10 tracking-widest uppercase">Base Reference</div>
             </div>
          </div>
          ...
          <div className="flex flex-col gap-3">
             <div className="flex-1 rounded-[2.5rem] bg-[#111113] border border-cyan-500/20 overflow-hidden relative shadow-[0_0_50px_rgba(34,211,238,0.15)] group">
                ...
                <div className="absolute top-6 left-1/2 -translate-x-1/2 px-4 py-1.5 bg-cyan-600/90 backdrop-blur-md rounded-2xl text-[10px] font-black text-white shadow-2xl border border-white/20 tracking-[0.2em] uppercase text-center">Difference Detection HUB</div>
             </div>
          </div>
          ...
          <div className="flex flex-col gap-3">
             <div className="flex-1 rounded-[2.5rem] bg-black border border-white/5 overflow-hidden relative shadow-2xl group">
                ...
                {activeProblem?.box && renderSpatialHub(activeProblem.box, 'simple')}
                <div className="absolute top-6 right-6 px-3 py-1 bg-purple-600/60 backdrop-blur-md rounded-xl text-[10px] font-bold text-white border border-white/10 tracking-widest uppercase">Compare Target</div>
             </div>
          </div>
        </section>
      </main>
