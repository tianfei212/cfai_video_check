  ...
  return (
    <div className="max-w-3xl mx-auto py-10 animate-in fade-in slide-in-from-bottom-4 duration-700">
      <div className="mb-12 text-center lg:text-left">
        <h1 className="text-3xl font-bold tracking-tight mb-2 text-[#1D1D1F]">{t("新建审核工程")}</h1>
        <p className="text-gray-400">{t("配置导入策略与 AI 检测主题，启动自动化合规线程")}</p>
      </div>
      ...
      <div className="space-y-8">
        ...
        <section className="apple-card p-8">
          <div className="flex items-center justify-between mb-6">
            <h3 className="text-sm font-bold text-gray-400 uppercase tracking-widest">{t("导入策略 (OSS 预处理)")}</h3>
            <span className="px-2 py-0.5 bg-blue-50 text-blue-500 text-[10px] font-bold rounded-md border border-blue-100 uppercase tracking-tighter">{t("当前推荐")}</span>
          </div>
          ...
          <div className="relative group">
            <div className="p-8 rounded-3xl border-2 border-black bg-gray-50/50 flex items-start gap-6 transition-all">
              ...
              <div className="flex-1">
                <h4 className="text-lg font-bold text-[#1D1D1F] mb-1">{t("直接视频流导入")}</h4>
                <p className="text-sm text-gray-400 leading-relaxed max-w-md">{t("适用于完整成片或粗剪素材。系统将自动进行 OSS 分片预处理，支持实时切片分析与并行推理计算。")}</p>
              </div>
              <div className="hidden sm:flex items-center gap-2">
                <div className="w-2 h-2 rounded-full bg-green-500"></div>
                <span className="text-[10px] font-bold text-gray-400 uppercase tracking-widest">{t("在线就绪")}</span>
              </div>
            </div>
          </div>
        </section>
        ...
        <section className="apple-card p-8">
          <div className="flex items-center justify-between mb-6">
            <h3 className="text-sm font-bold text-gray-400 uppercase tracking-widest">{t("AI 检测主题配置")}</h3>
            <div className="flex items-center gap-2">
               <span className="text-[10px] text-gray-300 font-bold uppercase tracking-widest">{t("已选检测项")}: {themes.length}</span>
            </div>
          </div>
          ...
          {/* 自动化流程步序区 */}
          <div className="mb-8">
            <p className="text-[10px] font-bold text-gray-300 uppercase tracking-widest mb-4">{t("自动化流程步序 (AUTOMATION WORKFLOW)")}</p>
            <div className="grid grid-cols-3 gap-4">
              ...
              <div className="relative group cursor-not-allowed">
                <div className="flex items-center justify-center h-14 rounded-2xl bg-black text-white shadow-lg border-2 border-black">
                  ...
                  <span className="text-xs font-bold italic">{t("视频导入")}</span>
                </div>
              ...
              >
                ...
                <div className="flex flex-col items-start">
                   <span className="text-xs font-bold italic">{t("自动切分视频")}</span>
                   {autoSegment && <span className="text-[7px] opacity-60 uppercase font-mono tracking-tighter">{t(segmentConfig.filmType)} @ {segmentConfig.fps}fps</span>}
                </div>
                {autoSegment && (
              ...
              >
                ...
                <span className="text-xs font-bold italic">{t("自动主体检测")}</span>
                {autoDetect && (
                  <div className="absolute -top-1.5 -right-1.5 w-5 h-5 bg-black rounded-full flex items-center justify-center border-2 border-white shadow-sm animate-in zoom-in duration-300">
                    <svg className="w-2.5 h-2.5 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="3" d="M5 13l4 4L19 7"/></svg>
                  </div>
          ...
          <div className="p-5 rounded-2xl bg-gray-50 border border-gray-100">
             <div className="flex items-center gap-3 mb-2">
                ...
                <span className="text-[10px] font-bold text-gray-500 uppercase tracking-widest">{t("核心原则 (CORE PRINCIPLE)")}</span>
             </div>
             <p className="text-[11px] text-gray-400 leading-relaxed italic">
                {autoSegment && autoDetect 
                  ? t("当前已开启完整自动化流程：导入视频后系统将立即启动云端切片，并基于所选主题进入自动检测环节。") 
                  : t("导入视频后，系统仅完成基础预处理。如需全自动检测，请在上方勾选相应的自动化流程步序。")}
             </p>
          </div>
        </section>
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-6 bg-black/40 backdrop-blur-md animate-in fade-in duration-300">
           <div className="w-full max-w-xl bg-white rounded-[2.5rem] shadow-3xl overflow-hidden animate-in zoom-in-95 duration-500">
              <div className="p-10 pb-6 border-b border-gray-50">
                 <h2 className="text-2xl font-bold tracking-tight mb-2">{t("自动切分引擎配置")}</h2>
                 <p className="text-xs text-gray-400 font-medium">{t("配置 AI 视觉推理模型的切点检测参数，以优化转场识别准确度。")}</p>
              </div>

              <div className="p-10 space-y-8">
                 {/* 影片类型 */}
                 <div className="space-y-3">
                    <label className="text-[10px] font-bold text-gray-400 uppercase tracking-widest ml-1">{t("影片艺术类型 (FILM GENRE)")}</label>
                 ...
                 <div className="space-y-3">
                    <div className="flex justify-between items-end ml-1">
                       <label className="text-[10px] font-bold text-gray-400 uppercase tracking-widest">{t("推理置信度阈值 (AI CONFIDENCE)")}</label>
                 ...
                 <div className="grid grid-cols-2 gap-8">
                    {/* 影片帧率 */}
                    <div className="space-y-3">
                       <label className="text-[10px] font-bold text-gray-400 uppercase tracking-widest ml-1">{t("影片输出帧率 (FPS)")}</label>
                       <select 
                    ...
                    {/* 黑场帧数 */}
                    <div className="space-y-3">
                       <label className="text-[10px] font-bold text-gray-400 uppercase tracking-widest ml-1">{t("黑场判定帧数 (BLACK THRESHOLD)")}</label>
                       <div className="flex items-center bg-gray-50 rounded-2xl border border-transparent focus-within:ring-4 focus-within:ring-black/5 transition-all">
                          ...
                          />
                          <span className="pr-5 text-[10px] font-bold text-gray-300">{t("FRAMES")}</span>
                       </div>
                    </div>
                 </div>
              </div>
