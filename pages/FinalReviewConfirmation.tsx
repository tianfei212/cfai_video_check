  ...
  if (isSuccess) {
    return (
      <div className="fixed inset-0 bg-[#0A0A0B] flex flex-col items-center justify-center text-white z-[100] animate-in fade-in zoom-out duration-700">
         ...
         <h1 className="text-3xl font-bold tracking-tight mb-2 text-center">{t('签发成功')}</h1>
         <p className="text-gray-500 text-sm text-center font-medium">{t('任务已归档，正在返回系统索引...')}</p>
      </div>
    );
  }
    ...
    <div className="fixed inset-0 bg-[#0A0A0B] text-white flex flex-col font-sans overflow-hidden select-none animate-in fade-in duration-500">
      ...
      <header className="h-16 border-b border-white/5 flex items-center justify-between px-10 bg-black/40 backdrop-blur-md shrink-0">
        <div className="flex items-center gap-4">
           ...
           <h1 className="text-lg font-bold tracking-tight">{t('终审签发确认中心')}</h1>
        </div>
      ...
      <main className="flex-1 overflow-y-auto custom-scrollbar p-10 flex flex-col items-center">
        <div className="w-full max-w-5xl space-y-12 animate-in slide-in-from-bottom-6 duration-700">
           ...
           <div className="text-center">
              <h2 className="text-5xl font-black tracking-tighter mb-4 text-white/95 text-balance">{t('审核报告核验就绪')}</h2>
              <p className="text-gray-400 text-lg max-w-2xl mx-auto font-medium">{t('您即将对影片进行专家级最终审定。签发后，该版本的合规序列将作为唯一官方存证结果同步至云端。')}</p>
           </div>

           {/* 统计看板 */}
           <div className="grid grid-cols-3 gap-6">
              <div className="p-8 rounded-[2.5rem] bg-white/5 border border-white/5 hover:border-white/10 transition-all group">
                 <p className="text-[10px] font-bold text-gray-500 uppercase tracking-widest mb-2 group-hover:text-blue-400">{t('AI 辅助发现')}</p>
                 <h4 className="text-4xl font-bold tracking-tighter">142 <span className="text-sm font-medium text-gray-500 ml-1">{t('项标记')}</span></h4>
              ...
              <div className="p-8 rounded-[2.5rem] bg-white/5 border border-white/5 hover:border-white/10 transition-all group">
                 <p className="text-[10px] font-bold text-gray-500 uppercase tracking-widest mb-2 group-hover:text-indigo-400">{t('人工标注补充')}</p>
                 <h4 className="text-4xl font-bold tracking-tighter">12 <span className="text-sm font-medium text-gray-500 ml-1">{t('项标记')}</span></h4>
              ...
              <div className="p-8 rounded-[2.5rem] bg-indigo-600/10 border border-indigo-500/20 hover:border-indigo-500/40 transition-all group">
                 <p className="text-[10px] font-bold text-indigo-400 uppercase tracking-widest mb-2">{t('综合合规建议')}</p>
                 <h4 className="text-4xl font-bold tracking-tighter text-indigo-100 italic underline decoration-indigo-500/30">{t('建议准予')}</h4>
              ...
              <div className="flex flex-col lg:flex-row gap-12">
                 ...
                 <div className="flex-1 space-y-4">
                    ...
                    <div className="grid grid-cols-2 gap-4">
                       <div className="p-4 rounded-2xl bg-black/40 border border-white/5">
                          <p className="text-[9px] font-bold text-gray-600 uppercase mb-1">{t('审核流水号 / ID')}</p>
                          <p className="text-xs font-bold font-mono text-gray-400">CAFAI-REF-20240203-09</p>
                       </div>
                       <div className="p-4 rounded-2xl bg-black/40 border border-white/5">
                          <p className="text-[9px] font-bold text-gray-600 uppercase mb-1">{t('系统当前时间')}</p>
                 ...
                 <div className="w-full lg:w-[360px] flex flex-col justify-between py-1">
                    <div className="space-y-4">
                       <label className="text-[10px] font-bold text-gray-500 uppercase tracking-widest ml-4">{t('在此处输入全名完成签名')}</label>
                       <div className="relative group">
                          <input 
                             type="text" 
                             value={signature}
                             onChange={(e) => setSignature(e.target.value)}
                             placeholder={t("请输入审核专家姓名")}
                    ...
                    <div className="mt-8 p-6 rounded-3xl bg-white/5 border border-white/5 flex flex-col gap-3">
                       <div className="flex justify-between items-center text-[10px] font-bold uppercase tracking-tighter">
                          <span className="text-gray-500">{t('签发人 ID')}</span>
                          <span className="text-indigo-400">EXPERT-ZH-202401</span>
                       </div>
                       <div className="w-full h-px bg-white/5"></div>
                       <div className="flex justify-between items-center text-[10px] font-bold uppercase tracking-tighter">
                          <span className="text-gray-500">{t('认证等级')}</span>
           ...
           <div className="flex flex-col items-center gap-6 pb-20">
              {isSubmitting ? (
                <div className="flex flex-col items-center gap-4">
                   ...
                   <p className="text-xs font-bold text-indigo-400 uppercase tracking-[0.2em]">{t('正在同步合规序列并签名归档...')}</p>
                </div>
              ) : (
                <>
                  <button 
                  ...
                  >
                    ...
                    <span className="relative z-10 group-hover:text-white transition-colors duration-500 italic">{t('确认签发并归档报告')}</span>
                  </button>
                  <button 
