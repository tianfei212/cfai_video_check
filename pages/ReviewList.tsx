...
const MOCK_DATA: FilmRecord[] = [
  { id: '1001', title: '流浪地球 3 预告片', thumbnail: 'https://picsum.photos/seed/earth/400/225', duration: '02:45', uploadDate: '2023-11-24', status: ReviewStatus.PROCESSING, riskLevel: RiskLevel.LOW, aiScore: 98, submitter: '中影集团' },
  { id: '1002', title: '未命名武侠大片', thumbnail: 'https://picsum.photos/seed/action/400/225', duration: '120:15', uploadDate: '2023-11-23', status: ReviewStatus.PENDING, riskLevel: RiskLevel.MEDIUM, aiScore: 72, submitter: '博纳影业' },
  { id: '1003', title: '深度空间：孤寂', thumbnail: 'https://picsum.photos/seed/space/400/225', duration: '88:40', uploadDate: '2023-11-23', status: ReviewStatus.FLAGGED, riskLevel: RiskLevel.HIGH, aiScore: 45, submitter: '独立制片人' },
];

// 模拟正在进行的后台任务
const ONGOING_TASKS = [
  { id: '1001', title: '流浪地球 3 预告片', phase: '自动切分', progress: 68, est: '12m' },
  { id: '1002', title: '未命名武侠大片', phase: '上传预处理', progress: 32, est: '4h' },
];
...
const StatusBadge: React.FC<{ status: ReviewStatus }> = ({ status }) => {
  ...
  return <span className={`px-2 py-0.5 rounded-md text-[9px] font-bold uppercase tracking-wider ${styles[status]}`}>{status}</span>;
};
  ...
  return (
    <div className="space-y-12">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-bold tracking-tight text-[#1D1D1F]">影片项目库</h1>
          <p className="text-gray-400 mt-1">管理并分析研究院所有在研影片合规工程</p>
        </div>
        <button 
        ...
        >
          ...
          <span className="text-sm font-bold">新建影片工程</span>
        </button>
      </div>
          ...
          <div key={film.id} className="apple-card group overflow-hidden flex flex-col cursor-pointer" onClick={() => navigate(`/project/${film.id}`)}>
            <div className="relative aspect-video overflow-hidden">
              ...
              <div className="absolute inset-0 bg-gradient-to-t from-black/80 via-transparent to-transparent opacity-0 group-hover:opacity-100 transition-opacity flex flex-col justify-end p-6">
                ...
                <p className="text-white/60 text-[10px] uppercase tracking-widest font-bold">点击管理版本序列</p>
              </div>
            ...
            <div className="p-6">
              ...
              <div className="flex items-center justify-between pt-6 border-t border-gray-50">
                <div className="flex flex-col">
                  <span className="text-[9px] font-bold text-gray-300 uppercase tracking-widest">最高风险</span>
                ...
                <div className="text-right flex flex-col">
                  <span className="text-[9px] font-bold text-gray-300 uppercase tracking-widest">迭代版本</span>
                  <span className="text-lg font-bold tracking-tighter">3 个</span>
                </div>
              </div>
            </div>
          </div>
        ...
        <div className="pt-10 border-t border-gray-100 animate-in fade-in slide-in-from-bottom-6 duration-1000">
          <div className="flex items-center justify-between mb-8">
            <div className="flex items-center gap-3">
              ...
              <h3 className="text-[10px] font-bold text-gray-400 uppercase tracking-[0.2em]">实时流水线监控 (REAL-TIME PIPELINE)</h3>
            </div>
            <button 
            ...
            {ONGOING_TASKS.map((task) => (
              ...
              >
                ...
                <div className="w-64">
                   ...
                   <p className="text-[9px] text-gray-400 font-bold uppercase tracking-widest">任务项 ID: CAFAI-{task.id}</p>
                </div>
                ...
                <div className="w-32 text-center">
                   <p className="text-[9px] font-bold text-gray-300 uppercase tracking-widest mb-0.5">剩余估时</p>
                   <p className="text-xs font-bold text-[#1D1D1F]">约 {task.est}</p>
                </div>
